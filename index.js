...
const WINDOW_RADIUS = { exterior: 39, interior: 26 };

const outer_plate_svg = `<g id="outer"><circle cx="0" cy="0" r="74" stroke="black" stroke-width=".25" fill="white" id="circle2" style="fill:#eee;" />`
  + FREQS_BY_10.flatMap(
    (freq) => NOTCHES.map(o => notch_on_outer_plate(o.length, freqToAngle(freq + o.delta)))
  ).join('')
  + FREQS_BY_10.map((freq) => text_on_outer_plate(freq, freqToAngle(freq))).join('') + `</g>`;


const inner_plate_svg = () => {

  const window = `M 0,-${WINDOW_RADIUS.interior} L 0,-${WINDOW_RADIUS.exterior} A ${WINDOW_RADIUS.exterior} ${WINDOW_RADIUS.exterior} 0 0 0 -${WINDOW_RADIUS.exterior * Math.SQRT1_2},-${WINDOW_RADIUS.exterior * Math.SQRT1_2} L -${WINDOW_RADIUS.interior * Math.SQRT1_2},-${WINDOW_RADIUS.interior * Math.SQRT1_2} A ${WINDOW_RADIUS.interior} ${WINDOW_RADIUS.interior} 0 0 1 0,-${WINDOW_RADIUS.interior}`;
  ...
  return `<g id="inner"><path fill="white" fill-rule="evenodd" d="M 0,-60 A 60 60 0 0 1 0,60 A 60 60 0 0 1 0,-60 ${window}" transform="rotate(22.5)" />`
    + FREQS_BY_10.flatMap(
      (freq) => NOTCHES.map(o => notch_on_inner_plate(o.length, freqToAngle(freq + o.delta)))
    ).join('')
